...
(() => {
  ...
  function initControlPanel(plusBtn) {
    _plusBtn2 = plusBtn;
    createEntry();
    createPanel();
    ensureControlPanel();
  ...
  function ensureControlPanel() {
    ...
    if (_panelOpen) {
      var r = root();
      if (_panel.parentNode !== r)
        r.appendChild(_panel);
      anchorPanel();
    }
  }
  function findRightPart() {
    return document.querySelector(".control-panel-icon-row.superChat .icon-right-part") || document.querySelector(".control-panel-icon-row .icon-right-part");
  }
  ...
  function setEntryFallbackStyle() {
    _entry.style.cssText = "position:fixed;right:16px;bottom:16px;z-index:" + (UI.Z_INDEX - 2) + ";width:36px;height:36px;border:1px solid rgba(255,255,255,.3);border-radius:8px;background:rgba(18,20,24,.9);color:#fff;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;box-shadow:0 4px 16px rgba(0,0,0,.35);opacity:.9;";
  }
  function createPanel() {
    _panel = document.createElement("div");
    _panel.dataset.dm1Panel = "1";
    _panel.style.cssText = "position:fixed;display:none;z-index:" + (UI.Z_INDEX - 2) + ';width:240px;padding:10px 12px;border:1px solid rgba(255,255,255,.16);border-radius:8px;background:rgba(18,20,24,.94);color:#e8eaed;font:12px/1.6 "PingFang SC","Microsoft YaHei",sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.45);user-select:none;';
    _panel.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;font-size:13px;font-weight:600;"><span>DM+1 \u8BBE\u7F6E</span><span data-dm1-panel-close style="cursor:pointer;color:#9aa0a6;font-size:14px;line-height:1;">\xD7</span></div><div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0;"><span>\u53D1\u9001\u51B7\u5374</span><input data-dm1-cooldown type="checkbox" style="accent-color:#fb7299;width:14px;height:14px;cursor:pointer;"></div><div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0;"><span>\u53D1\u9001\u95F4\u9694</span><select data-dm1-interval style="width:96px;background:#2a2d33;color:#e8eaed;border:1px solid rgba(255,255,255,.18);border-radius:6px;padding:2px 4px;"></select></div><div style="margin:6px 0;"><div style="margin-bottom:4px;">\u6309\u94AE\u900F\u660E\u5EA6</div><div data-dm1-opacity style="display:flex;gap:4px;"></div></div><div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0;"><span>\u8C03\u8BD5\u9762\u677F</span><input data-dm1-debug type="checkbox" style="accent-color:#fb7299;width:14px;height:14px;cursor:pointer;"></div><button data-dm1-reset style="width:100%;margin-top:4px;padding:5px 0;border:1px solid rgba(255,255,255,.18);border-radius:6px;background:transparent;color:#e8eaed;font-size:12px;cursor:pointer;">\u91CD\u7F6E\u6240\u6709\u8BBE\u7F6E</button><div style="margin-top:8px;text-align:right;color:#8a9199;font-size:11px;">' + getVersion() + "</div>";
    ...
    UI.OPACITY_OPTIONS.forEach(function(v) {
      ...
      btn.addEventListener("click", function() {
        ...
        if (_plusBtn2)
          _plusBtn2.style.opacity = String(v);
        syncOpacityButtons();
      });
  ...
  function onDocMouseDown(e) {
    if (_panel.contains(e.target) || _entry.contains(e.target))
      return;
    closePanel();
  }
  ...
  function onResetClick() {
    if (!_resetArmed) {
      ...
      _resetTimer = setTimeout(disarmReset, 3e3);
      return;
    }
